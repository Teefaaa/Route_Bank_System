...
Client::Client(string name, string password) : Person(name, password) {
  this->id = FilesHelper::getNextClientId();
  autoTransferDone = false;
  this->balance = 0;
  this->isActive = false;
...
bool Client::isTransferTimeAllowed() {
  ...
  return now.seconds_from(start) >= 0 && end.seconds_from(now) >= 0;
}
...
void Client::setActive(bool status) { isActive = status; }

bool Client::getActive() { return isActive; }

double Client::get_total_balances() const { return this->total_balances; }
double Client::get_total_loans() const { return this->total_loans; }
short Client::get_risk_counter() const { return this->risk_counter; }

double Client::get_loan() const { return this->loan; }
/* ================= AUTO TRANSACTIONS ================= */

void Client::auto_transfer_at(int hour, int minute, int second, double amount,
                              Client &recipient) {
  ...
  if (now.seconds_from(start) < 0 || end.seconds_from(now) < 0) {
    ...
    return;
    // this code executed only if the current time is between 00:00:00 and
    // 11:59:59
  }
  ...
  if (abs(now.seconds_from(target)) <= 1 && !autoTransferDone) {
    if (balance >= amount) {
      ...
      cout << "Auto transfer executed at " << hour << ":" << setw(2)
...
void Client::transfer_to(double amount, Client &recipient) {
  ...
  if (balance >= amount && recipient.getActive()) {
    ...
    if (transactionCount < MAX_TRANSACTIONS)
      transactions[transactionCount++] = Transaction("Transfer", amount);
    FilesHelper::saveAllClients();
...
void Client::request_loan() {
  ...
  if (transactionCount < MAX_TRANSACTIONS)
    transactions[transactionCount++] = Transaction("Loan", 10000);
...
void Client::check_balance() { cout << "Balance: " << balance << endl; }
...
void Client::show_transaction_history() {
  if (transactionCount == 0) {
    ...
    for (int i = 0; i < transactionCount; i++) {
      ...
      cout << transactions[i].get_type() << ": " << transactions[i].get_amount()
           << " at " << t.get_hours() << ":" << t.get_minutes() << ":"
           << t.get_seconds() << endl;
    }
  }
}
